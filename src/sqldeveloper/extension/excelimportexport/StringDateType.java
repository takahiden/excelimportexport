...
public class StringDateType extends StringDataType {
	...
	public void setSqlValue(Object value, int column, PreparedStatement statement)
		...
		if ((value instanceof String)) {
			...
			try {
				statement.setDate(column, new Date(format2.parse((String) value).getTime()));
				return;
			} catch (ParseException e) {
			}
			try {
				statement.setDate(column, new Date(format3.parse((String) value).getTime()));
				return;
			} catch (ParseException e) {
			}
			try {
				statement.setDate(column, new Date(format4.parse((String) value).getTime()));
				return;
			} catch (ParseException e) {
			}
			try {
				statement.setDate(column, new Date(format5.parse((String) value).getTime()));
				return;
			} catch (ParseException e) {
			}
			try {
				statement.setDate(column, new Date(format6.parse((String) value).getTime()));
				return;
			} catch (ParseException e) {
			}
